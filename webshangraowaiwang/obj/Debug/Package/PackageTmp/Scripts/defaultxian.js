...
jQuery(function ($) {
    $.get("Handler/WebSite.ashx?action=Filexian", { top: 10 }, function (data) {
        var tnewsTopFile = "";
        if (data != null) {
            var productData = data.T;
            $.each(productData, function (i, n) {
...
jQuery(function ($) {
    $.get("Handler/WebSite.ashx?action=IntenTop", { top: 7, IntentionType: 1 }, function (data) {
        ...
        var productData = data.T;
        $.each(productData, function (i, n) {
            tIntention += "<li><a href=\"IntenDetails.aspx?t=" + n.IntentionType + "&id=" + n.Id + "\">" + n.Title + "</a><span>" + n.CheckDate.substring(0, 10) + "</span></li>";
        });
...
//    
//    $.get("Handler/WebSite.ashx?action=BidListxian&flag=" + Math.random(), { rows: r }, function (data) {
//        var tbid = "";
//        var productData = data.T;
...
function OnShowBidClickByType(lb) {
    ...
    var code = $("#MainContent_hdcode").val();
    alert(code);
    $.get("Handler/WebSite.ashx?action=BidListxian&id=" + code + "&flag=" + Math.random(), { type: lb }, function (data) {
        var tbid = "";
        var productData = data.T;
        if (productData == null) { rowcount = productData.length; }
        $.each(productData, function (i, n) {
