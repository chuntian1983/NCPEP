...
jQuery(function ($) {
    $("#btnAuct").click(function () {
        $("#tLiceTranId").combobox('setValue', "");
        $("#tSeveralBid").combobox('setValue', "");
        $("#tFK_BidId").textbox('setValue', "");
        $("#tPrice").textbox('setValue', "");
        $('#BidClinch').window('open');
        var rows = $('#tdg').datagrid('getSelections');
        $("#tFK_BidId").textbox('setValue', rows[0].BidName);
        $("#tLiceTranId").combotree({
            url: '../Ashx/BidTran.ashx?' + $.param({ action: "combo", Id: rows[0].Id }),
            lines: true,
            required: true,
...
jQuery(function ($) {
    $("#btnAuctAdd").click(function () {
        var rows = $('#tdg').datagrid('getSelections');
        $.get("../Ashx/BidAucti.ashx?action=add", { LiceTranId: $("#tLiceTranId").combobox('getValue'), SeveralBid: $("#tSeveralBid").combobox('getValue'), FK_BidId: rows[0].Id, Price: $("#tPrice").textbox('getValue') }, function (data) {
            $('#BidClinch').window('close');
            msgShow("提示", data, "info");
            $("#btnAuct").hide();
            OnBidAuctionList(rows[0].Id);
